...
from cnn_train.cnn_dataset import (
    CNNJsonlGzSTLDataset,
    cnn_collate_records,
    decompress_gz_to_temp_jsonl,
)
from cnn_train.cnn_rasterizer import CNNRasterConfig
...
def compute_pos_weight(ds: CNNJsonlGzSTLDataset, *, max_items: Optional[int] = None) -> float:
    n_pos = 0
    n_neg = 0
    n_total = len(ds) if max_items is None else min(len(ds), int(max_items))
...
def main() -> None:
    ...
    p.add_argument("--pool_h", type=int, default=16)
    p.add_argument("--pool_w", type=int, default=16)
    p.add_argument("--mlp_hidden1", type=int, default=512)
    p.add_argument("--mlp_hidden2", type=int, default=512)
    p.add_argument("--enc_dim", type=int, default=256)
    p.add_argument("--head_hidden", type=int, default=256)

    p.add_argument("--ckpt_out", type=str, default="results/mlp_ckpt.pt")
    ...
    try:
        ...
        cfg = MLPModelConfig(
            ...
            pool_h=int(args.pool_h),
            pool_w=int(args.pool_w),
            mlp_hidden1=int(args.mlp_hidden1),
            mlp_hidden2=int(args.mlp_hidden2),
        ...
        for ep in range(int(args.epochs)):
            ...
            print(
                f"[mlp_train] epoch={ep + 1}/{int(args.epochs)} "
        ...
        print(f"[mlp_train] saved ckpt to: {out_ckpt}")
        ...
        for pth in [train_plain_path, val_plain_path]:
            ...
            if str(pth).endswith(".jsonl") and os.path.isfile(pth):
                try:
                    os.remove(pth)
                    print(f"[mlp_train] removed temp file: {pth}")
                except OSError:
                    pass
