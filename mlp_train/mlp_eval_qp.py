...
from gnn_train.eval_diag import DiagLogger, compute_face_switch_stats, status_name
...
from scripts import build_ast_fn, build_problem_fn, build_samples
from stl import STLBinaryExtractor, STLNode

from cnn_train.cnn_rasterizer import CNNRasterConfig, rasterize_env_to_image

from .mlp_model import MLPBinaryNet, MLPModelConfig


def load_mlp_from_ckpt(path: str, device: torch.device) -> Tuple[MLPBinaryNet, Dict[str, Any]]:
    ckpt = torch.load(str(path), map_location=device)
    if not isinstance(ckpt, dict):
        raise ValueError("MLP checkpoint must be a dict.")

    cfg_dict = ckpt.get("cfg", {})
    if not isinstance(cfg_dict, dict):
        raise ValueError("MLP checkpoint missing dict cfg.")

    cfg = MLPModelConfig(**cfg_dict)
    model = MLPBinaryNet(cfg).to(device)
...
def mlp_predict_logits_by_id(
    *,
    ...
    t_horizon = int(sample["T"])
    descs = STLBinaryExtractor(int(t_horizon)).extract(ast)
...
def main() -> None:
    ...
    p.add_argument("--png_dir", type=str, default="results/mlp_eval_results")
    ...
    model, ckpt = load_mlp_from_ckpt(args.ckpt, device=device)

    ex_eval = int(args.example_id)
    if int(ex_eval) not in (1, 2, 3, 4, 5):
        raise ValueError(f"mlp_eval_qp supports example_id 1..5. Got ex_eval={ex_eval}")
    ...
    diag.writeln("==== mlp_eval_qp diagnostics (overwrite) ====")
    ...
    for i, s in enumerate(samples):
        ...
        bin_logits_by_id = mlp_predict_logits_by_id(
            model=model,
            sample=s,
            ast=ast,
            device=device,
        ...
        faces_fixed = _extract_selected_faces_by_pred(
            descs=descs_check,
        ...
        for oi, o_rect in enumerate(obstacles):
            ...
            if pred not in faces_fixed:
                raise RuntimeError(
                    f"Missing faces for obstacle pred={pred}. Have: {list(faces_fixed.keys())[:10]}"
                )

            faces = faces_fixed[pred]
            prev0 = (
                int(faces[0])
                if isinstance(faces, np.ndarray) and faces.ndim == 1 and faces.shape[0] >= 1
                else 0
            )
        ...
        def _build_constraints_from_strategy(strat_local: Dict[str, int]):
            events_local = _extract_selected_event_times(
                descs=descs_check,
                strategy=strat_local,
            )

            in_rect_local: List[Tuple[int, Tuple[float, float, float, float], str]] = []
            outside_face_local: List[Tuple[int, Tuple[float, float, float, float], int, str]] = list(
                outside_face_obstacles
            )
            ...
            if int(ex_eval) == 1:
                sel = [
                    (pred, k, m)
                    for (pred, k, m) in events_local
                    if pred == "G" and not bool(m.get("until", False))
                ]
                if len(sel) != 1:
                    raise RuntimeError(
                        f"Example1: expected exactly 1 selected goal event_time for pred='G', got {sel}"
                    )
                _, k_goal, _ = sel[0]
                if "G" not in regions:
                ...
                if len(sel_g) != 1:
                    raise RuntimeError(
                        f"Example2: expected exactly 1 selected goal event_time for pred='G', got {sel_g}"
                    )
                _, k_goal, _ = sel_g[0]
                g_rect_local = regions["G"]
                in_rect_local.append((int(k_goal), g_rect_local, f"in_G_t{int(k_goal)}"))
                ...
                sel_d = [
                    (pred, k, m)
                    for (pred, k, m) in events_local
                    if bool(m.get("dwell_a", False)) or bool(m.get("dwell_b", False))
                ]
                if len(sel_d) != 1:
                    raise RuntimeError(
                        f"Example2: expected exactly 1 selected dwell event_time, got {sel_d}"
                    )
                pred_d, k_mid, _ = sel_d[0]

                tau = int(meta.get("tau", env.get("tau", None)))
                if tau is None:
                    raise RuntimeError(
                        "Example2: tau missing in env['meta']['tau'] (or legacy env['tau'])."
                    )
                ...
                if len(sel) != 1:
                    raise RuntimeError(
                        f"Example3: expected exactly 1 selected goal event_time among {goals}, got {sel}"
                    )
                ...
                in_rect_local.append(
                    (int(k_goal), regions[pred_g], f"in_{str(pred_g)}_t{int(k_goal)}")
                )
                g_rect_local = None
                ...
                for typ in types:
                    ...
                    if st not in picked:
                        raise RuntimeError(
                            f"Example4: missing selected event_time for type={st}. picked={picked}"
                        )
                ...
                extra_rects_local = [
                    (regions[k], k) for k in sorted(regions.keys()) if k.startswith("T_")
                ]
                g_rect_local = None
                ...
                if n_pairs < 1:
                    raise RuntimeError(
                        "Example5: expected at least one door/key pair named D0/K0,..."
                    )
                ...
                if len(sel_g) != 1:
                    raise RuntimeError(
                        f"Example5: expected exactly 1 selected goal event_time for pred='G', got {sel_g}"
                    )
                _, k_goal, _ = sel_g[0]
                in_rect_local.append((int(k_goal), g_rect_local, f"in_G_t{int(k_goal)}"))
                ...
                for idx in range(n_pairs):
                    ...
                    sel_k = [
                        (pred, k, m)
                        for (pred, k, m) in events_local
                        if pred == ki and bool(m.get("until", False))
                    ]
                    if len(sel_k) != 1:
                        raise RuntimeError(
                            f"Example5: expected exactly 1 selected until event_time for pred={ki}, got {sel_k}"
                        )
                    _, k_key, _ = sel_k[0]
                    k_key = int(k_key)
                    ...
                    if di not in faces_fixed:
                        raise RuntimeError(
                            f"Example5: missing faces for door pred={di}. Have: {list(faces_fixed.keys())[:10]}"
                        )
                    faces_d = faces_fixed[di]

                    prev0 = (
                        int(faces_d[0])
                        if isinstance(faces_d, np.ndarray)
                        and faces_d.ndim == 1
                        and faces_d.shape[0] >= 1
                        else 0
                    )

                    _force_start_consistent_faces_inplace(prob=prob, rect=d_rect, faces=faces_d)
                    forced_start_changed[di] = int(int(faces_d[0]) != int(prev0))

                    for t in range(0, k_key):
                        tag = f"outside_{di}_t{int(t)}_h{int(faces_d[t])}"
                        outside_face_local.append((t, d_rect, int(faces_d[t]), tag))
                    ...
                    for pred, mp in faces_override2.items():
                        ...
                        for t_str, h_new in mp.items():
                            ...
                            for j in range(len(outside_face_local)):
                                tj, rectj, _, tagj = outside_face_local[j]
                                if int(tj) != int(t):
                                    continue
                                ...
                                outside_face_local[j] = (
                                    int(t),
                                    rectj,
                                    int(h),
                                    f"outside_{pred}_t{int(t)}_h{int(h)}",
                                )
                                break
        ...
        if topk <= 1:
            ...
            x_sol, u_sol, t_qp, status_qp, iis_names = solve_qp_replay(
                prob=prob,
                in_rect=in_rect,
            ...
            solved_k = 1 if int(status_qp) == int(GRB.OPTIMAL) and x_sol is not None else -1
        else:
            ...
            x_sol, u_sol, t_qp, status_qp, iis_names, strategy, aux = solve_with_local_joint_topk(
                prob=prob,
                K=topk,
        ...
        if (int(status_qp) != int(GRB.OPTIMAL)) or (x_sol is None):
            x_fb, u_fb, t_fb = solve_with_stlpy_fallback(
                sample={"T": t_horizon, "env": s["env"]},
                example_id=int(ex_eval),
            )
        ...
        if int(status_qp) == int(GRB.OPTIMAL) and x_sol is not None:
            n_qp_opt += 1

            if int(ex_eval) == 1:
                avoid_ok, reach_ok, sat = check_example1(
                    X=x_sol,
                    obstacles=obstacles,
                    G_rect=regions["G"],
                )
                ...
                goals = [regions[k] for k in sorted(regions.keys()) if k.startswith("G")]
                avoid_ok, reach_ok, sat = check_example3(
                    X=x_sol,
                    obstacles=obstacles,
                    goals=goals,
                )
                ...
                for rk in regions.keys():
                    ...
                    if len(parts) == 3:
                        targets_by_type.setdefault(parts[1], []).append(regions[rk])
                avoid_ok, reach_all, sat = check_example4(
                    X=x_sol,
                    obstacles=obstacles,
                    targets_by_type=targets_by_type,
                )
                ...
                doors = [regions[k] for k in sorted(regions.keys()) if k.startswith("D")]
                keys = [regions[k] for k in sorted(regions.keys()) if k.startswith("K")]
                avoid_ok, reach_ok, doors_ok, sat = check_example5(
                    X=x_sol,
                    obstacles=obstacles,
                    doors=doors,
                    keys=keys,
            ...
            if u_sol is not None:
                cost_val = compute_solution_cost(prob=prob, X=x_sol, U=u_sol)
        else:
            qp_nonopt.append((int(i), int(status_qp)))
            n_nonopt += 1

        if str(args.success_metric) == "QP_opt":
            success_case = bool(int(status_qp) == int(GRB.OPTIMAL) and x_sol is not None)
        elif str(args.success_metric) == "SAT":
            success_case = bool(sat)
        else:
            success_case = bool(int(status_qp) == int(GRB.OPTIMAL) and x_sol is not None and sat)
        ...
        diag.writeln(
            f"status={status_str} ({int(status_qp)})  T={int(t_horizon)}  "
        ...
        if int(ex_eval) == 5:
            try:
                sel_g2 = [
                    (pred, k, m)
                    for (pred, k, m) in events
                    if pred == "G" and not bool(m.get("until", False))
                ]
                if len(sel_g2) == 1:
                    diag.writeln(f"ex5_k_goal={int(sel_g2[0][1])}")

                sel_k2 = [
                    (pred, k, m)
                    for (pred, k, m) in events
                    if pred.startswith("K") and bool(m.get("until", False))
                ]
                if len(sel_k2) > 0:
                    diag.writeln("ex5_k_keys:")
                    for pred, k, _ in sel_k2:
        ...
        if len(face_stats) > 0:
            ...
            for pred in sorted(face_stats.keys()):
                ...
                diag.writeln(
                    f"  - pred={pred}  switches={int(st['switches'])}  n_segments={int(st['n_segments'])}  forced_start_changed={ch}"
                )
        else:
        ...
        if int(status_qp) in (int(GRB.INFEASIBLE), int(GRB.INF_OR_UNBD)) and len(iis_names) > 0:
            diag.writeln(
                f"IIS (first {min(len(iis_names), int(args.diag_iis_limit))} constraints):"
            )
            for nm in iis_names:
    ...
    print(
        f"OPTIMAL+SAT: {n_opt_sat}/{denom}  OPTIMAL+UNSAT: {n_opt_unsat}/{denom}  NON-OPT: {n_nonopt}/{denom}"
    )
    ...
    diag.writeln(
        f"QP_opt={int(n_qp_opt)}  SAT={int(n_sat)}  avoid_ok={int(n_avoid)}  reach_ok={int(n_reach)}"
    )
    if int(ex_eval) in (2, 4, 5):
        diag.writeln(f"extra_ok={int(n_extra)}")
    diag.writeln(
        f"OPTIMAL+SAT={int(n_opt_sat)}  OPTIMAL+UNSAT={int(n_opt_unsat)}  NON-OPT={int(n_nonopt)}"
    )
